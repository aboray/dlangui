...
class Document : Element
{
    ...
    const
    {
        ...
        override bool opEquals(scope const Object o) const
        {
            ...
            return prolog == doc.prolog && (cast(const) this)
                .Element.opEquals(cast(const) doc) && epilog == doc.epilog;
        }
...
class Element : Item
{
     ...
     * --------------
     */
    void opOpAssign(string op)(Text item) @safe pure if (op == "~")
    {
        texts ~= item;
        appendItem(item);
    }
     ...
     * --------------
     */
    void opOpAssign(string op)(CData item) @safe pure if (op == "~")
    {
        cdatas ~= item;
        appendItem(item);
    }
     ...
     * --------------
     */
    void opOpAssign(string op)(Comment item) @safe pure if (op == "~")
    {
        comments ~= item;
        appendItem(item);
    }
     ...
     * --------------
     */
    void opOpAssign(string op)(ProcessingInstruction item) @safe pure if (op == "~")
    {
        pis ~= item;
        appendItem(item);
    }
     ...
     * --------------
     */
    void opOpAssign(string op)(Element item) @safe pure if (op == "~")
    {
        elements ~= item;
        appendItem(item);
    }
    ...
    override int opCmp(scope const Object o) @safe const
    {
        ...
        for (uint i = 0;; ++i)
        {
            if (i == items.length && i == element.items.length)
                return 0;
            if (i == items.length)
                return -1;
            if (i == element.items.length)
                return 1;
            if (!items[i].opEquals(element.items[i]))
    ...
    const
    {
        ...
        string text(DecodeMode mode = DecodeMode.LOOSE)
        {
            ...
            foreach (item; items)
            {
                ...
                if (t is null)
                    throw new DecodeException(item.toString());
        ...
        override string[] pretty(uint indent = 2) scope
        {
            ...
            if (items.length == 1)
            {
                ...
                {
                    return [
                        tag.toStartString() ~ t.toString() ~ tag.toEndString()
                    ];
                }
            }
        ...
        override @property @safe pure @nogc nothrow bool isEmptyXML() const scope
        {
...
class Tag
{
    ...
    invariant()
    {
        ...
        {
            assert(false, "Invalid tag name:" ~ e.toString());
        }
    ...
    const
    {
        ...
        override bool opEquals(scope Object o)
        {
            ...
            return (name != tag.name) ? false : ((attr != tag.attr)
                    ? false : ((type != tag.type) ? false : (true)));
        }
        ...
        override int opCmp(Object o)
        {
            ...
            return ((name != tag.name) ? (name < tag.name ? -1 : 1) : ((attr != tag.attr)
                    ? (cast(void*) attr < cast(void*) tag.attr ? -1 : 1) : ((type != tag.type)
                    ? (type < tag.type ? -1 : 1) : 0)));
        }
        ...
        private
        {
            ...
            string toStartString() @safe
            {
                return toNonEndString() ~ ">";
            }

            string toEndString() @safe
            {
                return "</" ~ name ~ ">";
            }

            string toEmptyString() @safe
            {
                return toNonEndString() ~ " />";
            }
        }
         ...
         * --------------
         */
        @property bool isStart() @safe @nogc pure nothrow
        {
         ...
         * --------------
         */
        @property bool isEnd() @safe @nogc pure nothrow
        {
         ...
         * --------------
         */
        @property bool isEmpty() @safe @nogc pure nothrow
        {
            return type == TagType.EMPTY;
        }
    }
}
...
class Comment : Item
{
     ...
     * can be used as associative array keys.
     */
    override size_t toHash() scope const nothrow
    {
        return hash(content);
    }
     ...
     */
    override string toString() scope const @safe pure nothrow
    {
        return "<!--" ~ content ~ "-->";
    }

    override @property @safe @nogc pure nothrow scope bool isEmptyXML() const
    {
        return false;
...
class CData : Item
{
    ...
    this(string content) @safe pure
    {
        ...
        if (content.indexOf("]]>") != -1)
            throw new CDataException(content);
        this.content = content;
    }
...
class Text : Item
{
     ...
     */
    override string toString() scope const @safe @nogc pure nothrow
    {
        return content;
    }
...
class ProcessingInstruction : Item
{
    ...
    override @property @safe @nogc pure nothrow bool isEmptyXML() scope const
    {
        return false;
    } /// Returns false always
}
...
class ElementParser
{
     ...
     */
    @property @safe @nogc pure nothrow const(Tag) tag() const
    {
        return tag_;
    }
     ...
     * --------------
     */
    @property @safe @nogc pure nothrow void onText(Handler handler)
    {
        textHandler = handler;
    }
     ...
     * --------------
     */
    @safe @nogc pure nothrow void onTextRaw(Handler handler)
    {
        rawTextHandler = handler;
    }
     ...
     * --------------
     */
    @property @safe @nogc pure nothrow void onCData(Handler handler)
    {
     ...
     * --------------
     */
    @property @safe @nogc pure nothrow void onComment(Handler handler)
    {
     ...
     * --------------
     */
    @property @safe @nogc pure nothrow void onPI(Handler handler)
    {
        piHandler = handler;
    }
     ...
     * --------------
     */
    @property @safe @nogc pure nothrow void onXI(Handler handler)
    {
        xiHandler = handler;
    }
    ...
    void parse()
    {
        ...
        while (s.length != 0)
        {
            if (startsWith(*s, "<!--"))
            {
                chop(*s, 4);
                t = chop(*s, indexOf(*s, "-->"));
                if (commentHandler.funcptr !is null)
                    commentHandler(t);
                chop(*s, 3);
            }
            else if (startsWith(*s, "<![CDATA["))
            {
                chop(*s, 9);
                t = chop(*s, indexOf(*s, "]]>"));
                if (cdataHandler.funcptr !is null)
                    cdataHandler(t);
                chop(*s, 3);
            }
            ...
            else if (startsWith(*s, "<"))
            {
                ...
                else if (tag_.isEnd)
                {
                    ...
                    immutable(char)* p = startTag.tagString.ptr + startTag.tagString.length;
...
private
{
    ...
    void checkName(ref string s, out string name) @safe pure  // rule 5
    {
        ...
        foreach (size_t i, dchar c; s)
        {
            ...
            if (i == 0)
                fail();
            if (c == '-' || c == '.' || isDigit(c) || isCombiningChar(c) || isExtender(c))
                continue;
            n = i;
            break;
        }
    ...
    void checkSDDecl(ref string s) @safe pure  // rule 32
    {
        ...
        int n = 0;
        if (s.startsWith("'yes'") || s.startsWith("\"yes\""))
            n = 5;
        else if (s.startsWith("'no'") || s.startsWith("\"no\""))
            n = 4;
        else
            fail("standalone attribute value must be 'yes', \"yes\"," ~ " 'no' or \"no\"");
        s = s[n .. $];
    }
    ...
    void checkEnd(string end, ref string s) @safe pure
    {
        ...
        if (n == -1)
            throw new Err(s, "Unable to find terminating \"" ~ end ~ "\"");
        s = s[n .. $];
        checkLiteral(end, s);
    }
...
void check(string s) @safe pure
{
    try
    {
        ...
        checkDocument(s);
        if (s.length != 0)
            throw new Err(s, "Junk found after document");
    }
    catch (Err e)
    {
        e.complete(s);
...
@system pure unittest
{
    ...
    {
        auto n = e.toString()
            .indexOf("end tag name \"genres\" differs" ~ " from start tag name \"genre\"");
        assert(n != -1);
    }
}
...
{
    private this(string msg) @safe pure
    {
        super(msg);
    }
}
...
class CheckException : XMLException
{
    ...
    override string toString() const @safe pure
    {
        ...
        string s;
        if (line != 0)
            s = format("Line %d, column %d: ", line, column);
        s ~= msg;
        s ~= '\n';
        if (err !is null)
            s = err.toString() ~ s;
...
private
{
    inout(T) toType(T)(inout Object o)
    {
        ...
        if (t is null)
        {
            throw new InvalidTypeException(
                    "Attempt to compare a " ~ T.stringof ~ " with an instance of another type");
        }
        return t;
    }
    ...
    immutable CharTable = [
        0x9, 0x9, 0xA, 0xA, 0xD, 0xD, 0x20, 0xD7FF, 0xE000, 0xFFFD, 0x10000,
        0x10FFFF
    ];
    immutable BaseCharTable = [
        ...
        0x017E, 0x0180, 0x01C3, 0x01CD, 0x01F0, 0x01F4, 0x01F5, 0x01FA, 0x0217,
        0x0250, 0x02A8, 0x02BB, 0x02C1, 0x0386, 0x0386, 0x0388, 0x038A,
        0x038C, 0x038C, 0x038E, 0x03A1, 0x03A3, 0x03CE, 0x03D0, 0x03D6, 0x03DA,
        ...
        0x04EE, 0x04F5, 0x04F8, 0x04F9, 0x0531, 0x0556, 0x0559, 0x0559, 0x0561,
        0x0586, 0x05D0, 0x05EA, 0x05F0, 0x05F2, 0x0621, 0x063A, 0x0641,
        ...
        0x09B0, 0x09B2, 0x09B2, 0x09B6, 0x09B9, 0x09DC, 0x09DD, 0x09DF,
        0x09E1, 0x09F0, 0x09F1, 0x0A05, 0x0A0A, 0x0A0F, 0x0A10, 0x0A13, 0x0A28,
        0x0A2A, 0x0A30, 0x0A32, 0x0A33, 0x0A35, 0x0A36, 0x0A38, 0x0A39,
        0x0A59, 0x0A5C, 0x0A5E, 0x0A5E, 0x0A72, 0x0A74, 0x0A85, 0x0A8B, 0x0A8D,
        0x0A8D, 0x0A8F, 0x0A91, 0x0A93, 0x0AA8, 0x0AAA, 0x0AB0, 0x0AB2,
        0x0AB3, 0x0AB5, 0x0AB9, 0x0ABD, 0x0ABD, 0x0AE0, 0x0AE0, 0x0B05, 0x0B0C,
        0x0B0F, 0x0B10, 0x0B13, 0x0B28, 0x0B2A, 0x0B30, 0x0B32, 0x0B33,
        0x0B36, 0x0B39, 0x0B3D, 0x0B3D, 0x0B5C, 0x0B5D, 0x0B5F, 0x0B61, 0x0B85,
        ...
        0x0C2A, 0x0C33, 0x0C35, 0x0C39, 0x0C60, 0x0C61, 0x0C85, 0x0C8C, 0x0C8E,
        0x0C90, 0x0C92, 0x0CA8, 0x0CAA, 0x0CB3, 0x0CB5, 0x0CB9, 0x0CDE,
