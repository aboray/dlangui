...
interface UnknownWindowMessageHandler
{
    ...
    bool onUnknownWindowMessage(HWND hwnd, UINT message, WPARAM wParam,
            LPARAM lParam, ref LRESULT result);
}
...
class Win32Window : Window
{
    ...
    override bool setWindowState(WindowState newState, bool activate = false,
            Rect newWindowRect = RECT_VALUE_IS_NOT_SET)
    {
        ...
        if (newWindowRect != RECT_VALUE_IS_NOT_SET
                && (newState == WindowState.normal || newState == WindowState.unspecified))
        {
            ...
            if (newWindowRect.top == int.min || newWindowRect.left == int.min)
            {
                ...
                {
                    // change size only
                    SetWindowPos(_hwnd, NULL, 0, 0, newWindowRect.right + 2 * GetSystemMetrics(SM_CXDLGFRAME),
                            newWindowRect.bottom + GetSystemMetrics(SM_CYCAPTION) + 2 * GetSystemMetrics(SM_CYDLGFRAME),
                            flags | SWP_NOMOVE);
                    rectChanged = true;
                    res = true;
                }
            ...
            else
            {
                if (newWindowRect.bottom != int.min && newWindowRect.right != int.min)
                {
                    ...
                    SetWindowPos(_hwnd, NULL, newWindowRect.left, newWindowRect.top,
                            newWindowRect.right + 2 * GetSystemMetrics(SM_CXDLGFRAME),
                            newWindowRect.bottom + GetSystemMetrics(SM_CYCAPTION) + 2 * GetSystemMetrics(SM_CYDLGFRAME),
                            flags);
                    rectChanged = true;
                    res = true;
                ...
                else
                {
                    ...
                    SetWindowPos(_hwnd, NULL, newWindowRect.left,
                            newWindowRect.top, 0, 0, flags | SWP_NOSIZE);
                    rectChanged = true;
                    res = true;
                }
            }
        ...
        if (rectChanged)
        {
            handleWindowStateChange(newState, Rect(newWindowRect.left == int.min ? _windowRect.left
                    : newWindowRect.left, newWindowRect.top == int.min ? _windowRect.top : newWindowRect.top,
                    newWindowRect.right == int.min ? _windowRect.right : newWindowRect.right,
    ...
    override protected void handleWindowStateChange(WindowState newState,
            Rect newWindowRect = RECT_VALUE_IS_NOT_SET)
    {
        if (_destroying)
            return;
    ...
    private bool onMouse(uint message, uint flags, short x, short y)
    {
        ...
        if (((message == WM_MOUSELEAVE) || (x < 0 || y < 0 || x >= _dx || y >= _dy))
                && _mouseTracking)
        {
    ...
    bool onKey(KeyAction action, uint keyCode, int repeatCount,
            dchar character = 0, bool syskey = false)
    {
        debug (KeyInput)
            Log.d("enter onKey action=", action, " keyCode=", keyCode, " char=", character, "(",
                    cast(int) character, ")", " syskey=", syskey,
                    "    _keyFlags=", "%04x"d.format(_keyFlags));
        KeyEvent event;
        if (syskey)
        ...
        if (action == KeyAction.KeyDown || action == KeyAction.KeyUp)
        {
            ...
            default:
                ...
                updateKeyFlags((GetKeyState(VK_LSHIFT) & 0x8000) != 0
                        ? KeyAction.KeyDown : KeyAction.KeyUp, KeyFlag.LShift, KeyFlag.RShift);
                updateKeyFlags((GetKeyState(VK_RSHIFT) & 0x8000) != 0
                        ? KeyAction.KeyDown : KeyAction.KeyUp, KeyFlag.RShift, KeyFlag.LShift);
                updateKeyFlags((GetKeyState(VK_LWIN) & 0x8000) != 0
                        ? KeyAction.KeyDown : KeyAction.KeyUp, KeyFlag.LMenu, KeyFlag.RMenu);
                updateKeyFlags((GetKeyState(VK_RWIN) & 0x8000) != 0
                        ? KeyAction.KeyDown : KeyAction.KeyUp, KeyFlag.RMenu, KeyFlag.LMenu);
                updateKeyFlags((GetKeyState(VK_LMENU) & 0x8000) != 0
                        ? KeyAction.KeyDown : KeyAction.KeyUp, KeyFlag.LAlt, KeyFlag.RAlt);
                updateKeyFlags((GetKeyState(VK_RMENU) & 0x8000) != 0
                        ? KeyAction.KeyDown : KeyAction.KeyUp, KeyFlag.RAlt, KeyFlag.LAlt);
            ...
            debug (KeyInput)
            {
                ...
                {
                    debug (KeyInput)
                        Log.d(" flags updated: onKey action=", action, " keyCode=", keyCode, " char=", character, "(",
                                cast(int) character, ")", " syskey=", syskey,
                                "    _keyFlags=", "%04x"d.format(_keyFlags));
                }
                //if (action == KeyAction.KeyDown)
        ...
        else if (action == KeyAction.Text && character != 0)
        {
            ...
            else
            {
                ...
                if ((newFlags & KeyFlag.Alt) && (newFlags & KeyFlag.Control))
                {
                    ...
                    debug (KeyInput)
                        Log.d(" flags updated for text: onKey action=", action,
                                " keyCode=", keyCode, " char=", character,
                                "(", cast(int) character, ")", " syskey=", syskey,
                                "    _keyFlags=", "%04x"d.format(_keyFlags));
                }
            ...
            if (_windowList[i] is w)
            {
                for (uint j = i + 1; j < _windowList.length; j++)
                {
                    if (_windowList[j].flags & WindowFlag.Modal
                            && _windowList[j].windowState != WindowState.hidden)
                        return true;
                }
                return false;
            }
...
extern (Windows) int DLANGUIWinMainProfile(string[] args)
{
    int result;
...
int myWinMainProfile(string[] args)
{
    ...
    if (!w32platform.registerWndClass())
    {
        MessageBoxA(null, "This program requires Windows NT!",
                "DLANGUI App".toStringz, MB_ICONERROR);
        return 0;
    }
...
extern (Windows) LRESULT WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_WINDOWPOSCHANGED:
        {
            if (window !is null)
            {
                ...
                else
                {
                    ...
                    window.handleWindowStateChange(state, Rect(pos.x, pos.y, dx, dy));
    ...
    case WM_MOUSEWHEEL:
        if (window !is null)
        {
            if (window.onMouse(message, cast(uint) wParam,
                    cast(short)(lParam & 0xFFFF), cast(short)((lParam >> 16) & 0xFFFF)))
                return 0; // processed
        }
    ...
    case WM_SYSKEYUP:
        if (window !is null)
        {
            ...
            if (window.onKey(message == WM_KEYDOWN || message == WM_SYSKEYDOWN
                    ? KeyAction.KeyDown : KeyAction.KeyUp, cast(uint) new_vk,
                    repeatCount, 0, message == WM_SYSKEYUP || message == WM_SYSKEYDOWN))
                return 0; // processed
        }
        break;
    case WM_UNICHAR:
