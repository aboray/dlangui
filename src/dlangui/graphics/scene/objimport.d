...
struct ObjModelImport
{
    ...
    void createMeshIfNotExist()
    {
        ...
        if (_txCount)
        {
            mesh = new Mesh(VertexFormat(VertexElementType.POSITION,
                    VertexElementType.NORMAL, /*VertexElementType.COLOR, */ VertexElementType
                    .TEXCOORD0));
    ...
    protected bool addTriangle(FaceIndex v1, FaceIndex v2, FaceIndex v3)
    {
        ...
        if (fmt.hasElement(VertexElementType.NORMAL))
        {
            ...
            fmt.set(data.ptr + vfloats, VertexElementType.NORMAL, v2[2]
                    ? normalForIndex(v2[2]) : normal);
            fmt.set(data.ptr + vfloats * 2, VertexElementType.NORMAL, v3[2]
                    ? normalForIndex(v3[2]) : normal);
        }
        int startVertex = mesh.addVertexes(data.ptr[0 .. vfloats * 3]);
        mesh.addPart(PrimitiveType.triangles, [
                cast(ushort)(startVertex + 0), cast(ushort)(startVertex + 1),
