...
struct CSSToken
{
    ...
    union
    {
        ...
        struct
        {
            long intValue = 0L; // for number and dimension
...
struct CSSTokenizer
{
    ...
    */
    bool skipWhiteSpace()
    {
        ...
        for (;;)
        {
            ...
            else if (ch == 0xEF && tokenEnd + 2 < src.length
                    && src.ptr[tokenEnd + 1] == 0xBF && src.ptr[tokenEnd + 2] == 0xBD)
            {
    ...
    */
    ...
    CSSToken nextToken()
    {
        CSSToken res;
        res.type = next();
        if (res.type == CSSTokenType.str || res.type == CSSTokenType.ident
                || res.type == CSSTokenType.atKeyword
                || res.type == CSSTokenType.url || res.type == CSSTokenType.func)
        {
            if (tokenText.length)
...
unittest
{
    ...
    tokenizer.start("ident-1{ }\n#id\n'blabla' \"bla bla 2\" -ident2*=12345 -.234e+5 "
            ...
            ~ "url(   'text.css'  )url(bad url)functionName()url( bla )" ~ "'\\30 \\31'");
...
class CSSParser
{
    ...
    CSSTokenType skipWhiteSpace()
    {
        while (currentToken.type == CSSTokenType.whitespace
                || currentToken.type == CSSTokenType.comment
                || currentToken.type == CSSTokenType.delim)
            next();
    ...
    ComponentValueNode[] parseComponentValueList(CSSTokenType endToken1 = CSSTokenType.eof,
            CSSTokenType endToken2 = CSSTokenType.eof)
    {
        ...
        for (;;)
        {
            ...
            if (type == endToken1 || type == endToken2)
                return res;
            if (type == CSSTokenType.squareOpen
                    || type == CSSTokenType.parentOpen || type == CSSTokenType.curlyOpen)
            {
    ...
    ATRuleNode parseATRule()
    {
        ...
        res.componentValues = parseComponentValueList(CSSTokenType.semicolon,
                CSSTokenType.curlyOpen);
        type = skipWhiteSpace();
        if (type == CSSTokenType.semicolon)
        {
            next();
...
unittest
{
    QualifiedRuleNode qualifiedRule = new CSSParser(tokenizeCSS(" pre { display: none } "))
        .parseQualifiedRule();
