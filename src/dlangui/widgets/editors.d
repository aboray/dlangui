...
const Action ACTION_EDITOR_COPY = (new Action(EditorActions.Copy,
        "MENU_EDIT_COPY"c, null, KeyCode.KEY_C, KeyFlag.Control)).addAccelerator(
        KeyCode.INS, KeyFlag.Control).disableByDefault();
const Action ACTION_EDITOR_PASTE = (new Action(EditorActions.Paste,
        "MENU_EDIT_PASTE"c, null, KeyCode.KEY_V, KeyFlag.Control)).addAccelerator(
        KeyCode.INS, KeyFlag.Shift).disableByDefault();
const Action ACTION_EDITOR_CUT = (new Action(EditorActions.Cut,
        "MENU_EDIT_CUT"c, null, KeyCode.KEY_X, KeyFlag.Control)).addAccelerator(
        KeyCode.DEL, KeyFlag.Shift).disableByDefault();
const Action ACTION_EDITOR_UNDO = (new Action(EditorActions.Undo,
        "MENU_EDIT_UNDO"c, null, KeyCode.KEY_Z, KeyFlag.Control)).disableByDefault();
const Action ACTION_EDITOR_REDO = (new Action(EditorActions.Redo,
        "MENU_EDIT_REDO"c, null, KeyCode.KEY_Y, KeyFlag.Control)).addAccelerator(
        KeyCode.KEY_Z, KeyFlag.Control | KeyFlag.Shift).disableByDefault();

const Action ACTION_EDITOR_INSERT_NEW_LINE = (new Action(EditorActions.InsertNewLine,
        KeyCode.RETURN, 0, ActionStateUpdateFlag.never)).addAccelerator(
        KeyCode.RETURN, KeyFlag.Shift);
const Action ACTION_EDITOR_PREPEND_NEW_LINE = (new Action(EditorActions.PrependNewLine,
        KeyCode.RETURN, KeyFlag.Control | KeyFlag.Shift, ActionStateUpdateFlag.never));
const Action ACTION_EDITOR_APPEND_NEW_LINE = (new Action(EditorActions.AppendNewLine,
        KeyCode.RETURN, KeyFlag.Control, ActionStateUpdateFlag.never));
const Action ACTION_EDITOR_DELETE_LINE = (new Action(EditorActions.DeleteLine,
        KeyCode.KEY_D, KeyFlag.Control, ActionStateUpdateFlag.never)).addAccelerator(
        KeyCode.KEY_L, KeyFlag.Control);
const Action ACTION_EDITOR_TOGGLE_REPLACE_MODE = (new Action(
        EditorActions.ToggleReplaceMode, KeyCode.INS, 0, ActionStateUpdateFlag.never));
const Action ACTION_EDITOR_SELECT_ALL = (new Action(EditorActions.SelectAll,
        KeyCode.KEY_A, KeyFlag.Control, ActionStateUpdateFlag.never));
const Action ACTION_EDITOR_TOGGLE_LINE_COMMENT = (
        new Action(EditorActions.ToggleLineComment, KeyCode.KEY_DIVIDE, KeyFlag.Control));
const Action ACTION_EDITOR_TOGGLE_BLOCK_COMMENT = (new Action(EditorActions.ToggleBlockComment,
        KeyCode.KEY_DIVIDE, KeyFlag.Control | KeyFlag.Shift));
const Action ACTION_EDITOR_TOGGLE_BOOKMARK = (new Action(EditorActions.ToggleBookmark,
        "ACTION_EDITOR_TOGGLE_BOOKMARK"c, null, KeyCode.KEY_B, KeyFlag.Control | KeyFlag.Shift));
const Action ACTION_EDITOR_GOTO_NEXT_BOOKMARK = (new Action(EditorActions.GoToNextBookmark,
        "ACTION_EDITOR_GOTO_NEXT_BOOKMARK"c, null, KeyCode.DOWN,
        KeyFlag.Control | KeyFlag.Shift | KeyFlag.Alt));
const Action ACTION_EDITOR_GOTO_PREVIOUS_BOOKMARK = (new Action(EditorActions.GoToPreviousBookmark,
        "ACTION_EDITOR_GOTO_PREVIOUS_BOOKMARK"c, null, KeyCode.UP,
        KeyFlag.Control | KeyFlag.Shift | KeyFlag.Alt));
const Action ACTION_EDITOR_FIND = (new Action(EditorActions.Find,
        "ACTION_EDITOR_FIND"c, null, KeyCode.KEY_F, KeyFlag.Control));
const Action ACTION_EDITOR_FIND_NEXT = (new Action(EditorActions.FindNext,
        "ACTION_EDITOR_FIND_NEXT"c, null, KeyCode.F3, 0));
const Action ACTION_EDITOR_FIND_PREV = (new Action(EditorActions.FindPrev,
        "ACTION_EDITOR_FIND_PREV"c, null, KeyCode.F3, KeyFlag.Shift));
const Action ACTION_EDITOR_REPLACE = (new Action(EditorActions.Replace,
        "ACTION_EDITOR_REPLACE"c, null, KeyCode.KEY_H, KeyFlag.Control));

const Action[] STD_EDITOR_ACTIONS = [
    ACTION_EDITOR_INSERT_NEW_LINE, ACTION_EDITOR_PREPEND_NEW_LINE,
    ACTION_EDITOR_APPEND_NEW_LINE, ACTION_EDITOR_DELETE_LINE,
    ACTION_EDITOR_TOGGLE_REPLACE_MODE, ACTION_EDITOR_SELECT_ALL,
    ACTION_EDITOR_TOGGLE_LINE_COMMENT, ACTION_EDITOR_TOGGLE_BLOCK_COMMENT,
    ACTION_EDITOR_TOGGLE_BOOKMARK, ACTION_EDITOR_GOTO_NEXT_BOOKMARK,
    ACTION_EDITOR_GOTO_PREVIOUS_BOOKMARK, ACTION_EDITOR_FIND,
    ACTION_EDITOR_REPLACE, ACTION_EDITOR_FIND_NEXT, ACTION_EDITOR_FIND_PREV,
...
class EditWidgetBase : ScrollWidgetBase, EditableContentListener, MenuItemActionHandler
{
    ...
    @property bool copyCurrentLineWhenNoSelection()
    {
        return _copyCurrentLineWhenNoSelection;
    }

    @property EditWidgetBase copyCurrentLineWhenNoSelection(bool flg)
    {
        _copyCurrentLineWhenNoSelection = flg;
        return this;
    }

    protected bool _showTabPositionMarks = false;
    /// when true shows mark on tab positions in beginning of line
    @property bool showTabPositionMarks()
    {
    ...
    protected void updateLeftPaneWidth()
    {
        ...
        _modificationMarksWidth = _showModificationMarks && (BACKEND_GUI
                || !_showLineNumbers) ? _modificationMarksPaneWidth : 0;
        _lineNumbersWidth = 0;
        if (_showLineNumbers)
        {
    ...
    protected void drawLeftPaneIcons(DrawBuf buf, Rect rc, int line)
    {
        ...
        drawLeftPaneIcon(buf, rc, content.lineIcons.findByLineAndType(line,
                LineIconType.bookmark));
        drawLeftPaneIcon(buf, rc, content.lineIcons.findByLineAndType(line,
                LineIconType.breakpoint));
    }
    ...
    protected void drawLeftPaneLineNumbers(DrawBuf buf, Rect rc, int line)
    {
        ...
        if (line == _caretPos.line
                && !isFullyTransparentColor(_leftPaneLineNumberBackgroundColorCurrentLine))
    ...
    this(string ID, ScrollBarMode hscrollbarMode = ScrollBarMode.Visible,
            ScrollBarMode vscrollbarMode = ScrollBarMode.Visible)
    {
        ...
        acceleratorMap.add([
                ...
                new Action(EditorActions.Redo, KeyCode.KEY_Z,
    ...
    override void showPopupMenu(int x, int y)
    {
        ...
        popupMenu.menuItemAction = this;
        PopupWidget popup = window.showPopup(popupMenu, this,
                PopupAlign.Point | PopupAlign.Right, x, y);
        popup.flags = PopupFlags.CloseOnClickOutside;
    }
    ...
    mixin(generatePropertySettersMethodOverride("setBoolProperty", "bool",
            "wantTabs", "showIcons", "showFolding", "showModificationMarks",
            "showLineNumbers", "readOnly", "replaceMode", "useSpacesForTabs",
            "copyCurrentLineWhenNoSelection", "showTabPositionMarks"));
    ...
    /// set smart indents enabled flag
    @property EditWidgetBase smartIndentsAfterPaste(bool enabled)
    {
    ...
    override void onContentChange(EditableContent content, EditOperation operation,
    ...
    override void onThemeChanged()
    {
        ...
        _leftPaneLineNumberColorEdited = style.customColor(
                "editor_left_pane_line_number_text_edited", 0xC0C000);
        _leftPaneLineNumberColorSaved = style.customColor(
                "editor_left_pane_line_number_text_saved", 0x00C000);
        _leftPaneLineNumberColorCurrentLine = style.customColor(
                "editor_left_pane_line_number_text_current_line", 0xFFFFFFFF);
        _leftPaneLineNumberBackgroundColorCurrentLine = style.customColor(
        ...
        _colorIconBreakpoint = style.customColor(
                "editor_left_pane_line_icon_color_breakpoint", 0xFF0000);
        _colorIconBookmark = style.customColor("editor_left_pane_line_icon_color_bookmark",
                0x0000FF);
    ...
    protected void drawCaret(DrawBuf buf)
    {
        if (focused)
        {
            ...
            if (caretRc.intersects(_clientRect))
            {
                ...
                buf.fillRect(Rect(caretRc.left, caretRc.top, caretRc.left + 1,
                        caretRc.bottom), _caretColor);
            }
        }
    }
    ...
    {
        EditOperation op = new EditOperation(EditAction.Replace, _selectionRange, [
                newText
                ]);
    ...
    override protected bool handleAction(const Action a)
    {
        ...
        case SelectWordLeft:
            {
                ...
                {
                    _caretPos = newpos;
                    updateSelectionAfterCursorMovement(oldCaretPos,
                            a.id == EditorActions.SelectWordLeft);
                    ensureCaretVisible();
                }
            }
            return true;
        ...
        case SelectWordRight:
            {
                ...
                if (newpos != _caretPos)
                {
                    _caretPos = newpos;
                    updateSelectionAfterCursorMovement(oldCaretPos,
                            a.id == EditorActions.SelectWordRight);
                    ensureCaretVisible();
                }
            }
            return true;
        ...
        case Cut:
            ...
            if (!range.empty)
            {
                ...
                {
                    EditOperation op = new EditOperation(EditAction.Replace, range, [
                            ""d
                            ]);
                    _content.performOperation(op, this);
                }
        ...
        case Tab:
            {
                ...
                if (_selectionRange.empty)
                {
                    if (useSpacesForTabs)
                    {
                        ...
                        EditOperation op = new EditOperation(EditAction.Replace,
                                TextRange(_caretPos, _caretPos), [
                                    spacesForTab(_caretPos.pos)
                                ]);
                        _content.performOperation(op, this);
                    }
                    else
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (useSpacesForTabs)
                        {
                            ...
                            EditOperation op = new EditOperation(EditAction.Replace,
                                    _selectionRange, [
                                        spacesForTab(_selectionRange.start.pos)
                                    ]);
                            _content.performOperation(op, this);
                        }
                        else
        ...
        case BackTab:
            {
                ...
                if (_selectionRange.empty)
                {
                    ...
                    if (!r.empty)
                    {
                        EditOperation op = new EditOperation(EditAction.Replace, r, [
                                ""d
                                ]);
                        _content.performOperation(op, this);
                    }
        ...
        case GoToPreviousBookmark:
            if (_content.multiline)
            {
                LineIcon mark = _content.lineIcons.findNext(LineIconType.bookmark,
                        _selectionRange.end.line, a.id == EditorActions.GoToNextBookmark ? 1 : -1);
                if (mark)
                {
    ...
    protected void indentRange(bool back)
    {
        ...
        for (int i = 0; i < lineCount; i++)
        {
            ...
            dstring dstline = indentLine(srcline, back,
                    r.start.line + i == _caretPos.line ? &_caretPos : null);
    ...
    override protected Action findKeyAction(uint keyCode, uint flags)
    {
        ...
        if (keyCode == KeyCode.TAB && (flags == 0 || flags == KeyFlag.Shift)
                && (!_wantTabs || readOnly))
            return null;
    ...
    override bool onKeyEvent(KeyEvent event)
    {
        ...
        if (event.action == KeyAction.Text && event.text.length && !ctrlOrAltPressed)
        {
            ...
            if (readOnly)
                return true;
            if (!(!!(event.flags & KeyFlag.Alt) && event.text.length == 1 && isAZaz(event.text[0])))
            { // filter out Alt+A..Z
    ...
    override bool onMouseEvent(MouseEvent event)
    {
        ...
        // support onClick
        bool insideLeftPane = event.x < _clientRect.left
            && event.x >= _clientRect.left - _leftPaneWidth;
        ...
        if (event.action == MouseAction.ButtonDown && event.button == MouseButton.Left)
        {
            ...
            else
            {
                ...
                updateCaretPositionByMouse(event.x - _clientRect.left,
                        event.y - _clientRect.top, doSelect);
...
class EditLine : EditWidgetBase
{
    ...
    override protected Point measureVisibleText()
    {
        ...
        int charsMeasured = font.measureText(_measuredText,
                _measuredTextWidths, MAX_WIDTH_UNSPECIFIED, tabSize);
    ...
    protected Point measureTextToSetWidgetSize()
    {
        ...
        int charsMeasured = font.measureText(_textToSetWidgetSize,
                _measuredTextToSetWidgetSizeWidths, MAX_WIDTH_UNSPECIFIED, tabSize);
        _measuredTextToSetWidgetSize.x = charsMeasured > 0
            ? _measuredTextToSetWidgetSizeWidths[charsMeasured - 1] : 0;
    ...
    override void measure(int parentWidth, int parentHeight)
    {
        ...
        measuredContent(parentWidth, parentHeight,
                _measuredTextToSetWidgetSize.x + _leftPaneWidth, _measuredTextToSetWidgetSize.y);
    }
...
class EditBox : EditWidgetBase
{
    ...
    /// create with ID parameter
    this(string ID, dstring initialContent = null, ScrollBarMode hscrollbarMode = ScrollBarMode.Visible,
            ScrollBarMode vscrollbarMode = ScrollBarMode.Visible)
    {
        super(ID, hscrollbarMode, vscrollbarMode);
    ...
    override @property int fontSize() const
    {
    ...
    override protected Point measureVisibleText()
    {
        ...
        for (int i = 0; i < _numVisibleLines; i++)
        {
            ...
            int charsMeasured = font.measureText(_visibleLines[i],
                    _visibleLinesMeasurement[i], int.max, tabSize);
            _visibleLinesWidths[i] = charsMeasured > 0 ? _visibleLinesMeasurement[i][charsMeasured
                - 1] : 0;
    ...
    {
        needHScroll = _hscrollbar && (_hscrollbarMode == ScrollBarMode.Visible
                || _hscrollbarMode == ScrollBarMode.Auto);
        needVScroll = _vscrollbar && (_vscrollbarMode == ScrollBarMode.Visible
        ...
        if (visibleLines < 1)
            visibleLines = 1;
        int visibleLinesWithScrollbar = _lineHeight > 0 ? (
                (_clientRect.height - hsbHeight) / _lineHeight) : 1; // fully visible lines
    ...
    {
        _hscrollbar.setRange(0, _maxLineWidth + (_extendRightScrollBound
                ? _clientRect.width / 16 : 0));
    ...
    override protected bool handleAction(const Action a)
    {
        ...
        case ZoomIn:
            {
                ...
                if (_minFontSize < _maxFontSize && _minFontSize > 0 && _maxFontSize > 0)
                {
                    ...
                    if (currentFontSize != newFontSize
                            && newFontSize <= _maxFontSize && newFontSize >= _minFontSize)
                    {
        ...
        case DeleteLine:
            if (!readOnly)
            {
                correctCaretPos();
                EditOperation op = new EditOperation(EditAction.Replace,
                        _content.lineRange(_caretPos.line), [""d]);
                _content.performOperation(op, this);
            }
            return true;
        case Find:
    ...
    protected void highlightTextPattern(DrawBuf buf, int lineIndex, Rect lineRect, Rect visibleRect)
    {
        ...
        for (;;)
        {
            ptrdiff_t pos = lineText[start .. $].indexOf(pattern,
                    caseSensitive ? Yes.caseSensitive : No.caseSensitive);
            if (pos < 0)
                break;
            ...
            if (!wholeWords || isWholeWord(lineText, start, start + pattern.length))
            {
                ...
                uint color = r.isInsideOrNext(caretPos)
                    ? _searchHighlightColorCurrent : _searchHighlightColorOther;
    ...
    TextRange[] findAll(dstring pattern, uint options)
    {
        ...
        for (int i = 0; i < _content.length; i++)
        {
            ...
            for (;;)
            {
                ...
                if (!wholeWords || isWholeWord(lineText, start, start + pattern.length))
                {
                    TextRange r = TextRange(TextPosition(i, cast(int) start),
                            TextPosition(i, cast(int)(start + pattern.length)));
                    res ~= r;
                }
    ...
    void wordWrapFillRect(DrawBuf buf, int line, Rect lineToDivide, uint color)
    {
        ...
        foreach (n, wrapLineRect; wrappedSelection)
        {
            int i = cast(int) n;
            int startingDifference = rc.left - _clientRect.left;
            wrapLineRect = rc;
            wrapLineRect.offset(-1 * curSpan.accumulation(i,
                    LineSpan.WrapPointInfo.Width), i * _lineHeight);
    ...
    protected void drawLineBackground(DrawBuf buf, int lineIndex, Rect lineRect, Rect visibleRect)
    {
        ...
        if (!_selectionRange.empty && _selectionRange.start.line <= lineIndex
                && _selectionRange.end.line >= lineIndex)
        {
            // line inside selection
            Rect startrc = textPosToClient(_selectionRange.start);
            Rect endrc = textPosToClient(_selectionRange.end);
            int startx = lineIndex == _selectionRange.start.line
                ? startrc.left + _clientRect.left : lineRect.left;
            int endx = lineIndex == _selectionRange.end.line
                ? endrc.left + _clientRect.left : lineRect.right + _spaceWidth;
            Rect rc = lineRect;
            rc.left = startx;
            rc.right = endx;
            if (!rc.empty && _wordWrap)
            {
                wordWrapFillRect(buf, lineIndex, rc, focused
                        ? _selectionColorFocused : _selectionColorNormal);
            }
            else if (!rc.empty)
            {
                // draw selection rect for line
    ...
    void setTokenHightlightColor(ubyte tokenCategory, uint color,
    ...
    @property bool showWhiteSpaceMarks() const
    {
    ...
    void drawWhiteSpaceMarks(DrawBuf buf, ref FontRef font, dstring txt,
            int tabSize, Rect lineRect, Rect visibleRect)
    {
        ...
        int charsMeasured = font.measureText(txt, textSizeBuffer,
                MAX_WIDTH_UNSPECIFIED, tabSize, 0, 0);
        int ts = tabSize;
        if (ts < 1)
            ts = 1;
        if (ts > 8)
        ...
        for (int i = 0; i < _visibleLines.length; i++)
        {
            ...
            if (txt.length > 0 || _wordWrap)
            {
                ...
                if (_wordWrap)
                {
                    ...
                    foreach (n, curWrap; wrappedLine)
                    {
                        ...
                        if (highlight)
                        {
                            ...
                            font.drawColoredText(buf, rc.left - _scrollPos.x,
                                    rc.top + lineOffset * _lineHeight, curWrap, wrapProps, tabSize);
                        }
                        else
                            font.drawText(buf, rc.left - _scrollPos.x,
                                    rc.top + lineOffset * _lineHeight, curWrap, textColor, tabSize);
                ...
                else
                {
                    if (highlight)
                        font.drawColoredText(buf, rc.left - _scrollPos.x,
                                rc.top + i * _lineHeight, txt, highlight, tabSize);
                    else
                        font.drawText(buf, rc.left - _scrollPos.x,
                                rc.top + i * _lineHeight, txt, textColor, tabSize);
                }
            }
        }
...
class LogWidget : EditBox
{
    ...
    /// when true, automatically scrolls down when new lines are appended (usually being reset by scrollbar interaction)
    @property void scrollLock(bool flg)
    {
        _scrollLock = flg;
    }
...
class FindPanel : HorizontalLayout
{
    ...
    bool findNext(bool back)
    {
        ...
        if (res)
        {
            _editor.selectionRange = TextRange(pos, TextPosition(pos.line,
                    pos.pos + cast(int) currentText.length));
    ...
    bool replaceOne()
    {
        ...
        if (res)
        {
            ...
            _editor.selectionRange = TextRange(pos, TextPosition(pos.line,
                    pos.pos + cast(int) newText.length));
            _editor.ensureCaretVisible();
            //_editor.setCaretPos(pos.line, pos.pos, true);
        }
        return res;
